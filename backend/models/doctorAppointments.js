...
//     GROUP BY 
//       DATE_TRUNC('day', da.available_start_date)
//     ORDER BY 
//       day
//   `;
...
const Patient = {
  updatePatientInfo: async (patientId, updates) => {
    ...
    // تحويل أي قيمة "null" string أو undefined لـ null حقيقي
    const sanitize = (val) =>
    ...
    return await db.query(
      `UPDATE patients
       SET chronic_diseases = $1, blood_type = $2, haveAllergy = $3
       WHERE id = $4`,
      [sanitize(chronic_diseases), sanitize(blood_type), sanitize(haveallergy), patientId]
    );
  },
};
