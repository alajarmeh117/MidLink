...
exports.getDoctorById = async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      "SELECT staff_id, staff_name, email, profile_image, specialty, bio, clinic_address FROM medical_staff WHERE staff_id = $1 AND is_approved = TRUE",
      [id],
    );
    ...
    res
      .status(500)
      .json({ error: "An error occurred while fetching the doctor" });
  }
};
...
exports.contactDoctor = async (req, res) => {
  ...
  try {
    ...
    const doctorResult = await db.query(
      "SELECT email FROM medical_staff WHERE staff_id = $1",
      [doctorId],
    );
