...
exports.getTodayAppointments = async (req, res) => {
  ...
  try {
    ...
    const staffId = decoded.id;
    const appointments = await getTodayDoctorAppointments(staffId);
    const scheduledAppointmentsCount = await getTodayScheduledAppointmentsCount(staffId);

    res.json({ scheduledAppointmentsCount, appointments });
  } catch (error) {
    console.error("Error in getTodayAppointments:", error);
    res.status(500).json({ message: "Server error" });
  }
...
exports.getWeeklyAppointments = async (req, res) => {
  ...
  try {
    ...
    const staffId = decoded.id;
    const weeklyData = await getWeeklyAppointmentsData(staffId);

    res.json({ weeklyData });
  } catch (error) {
    console.error("Error in getWeeklyAppointments:", error);
    res.status(500).json({ message: "Server error" });
  }
};
