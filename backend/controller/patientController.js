...
exports.getProfile = async (req, res) => {
  try {
    ...
    const profileData = {
      ...
      appointment_count:
        appointments.length > 0 ? appointments[0].appointment_count : 0,
    };
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    query +=
      " WHERE id = $" +
      (values.length + 1) +
      " RETURNING id, username, email, gender, dob, profile_image";
    values.push(id);
...
exports.updateProfileImage = async (req, res) => {
  try {
    ...
    const query =
      "UPDATE patients SET profile_image = $1 WHERE id = $2 RETURNING id, profile_image";
